...
export async function createStoreApi(payload: Record<string, any>): Promise<{ success: boolean; data?: any; error?: string }> {
  try {
    ...
    if (!res.ok || !json.success) {
      return { success: false, error: json.error?.message || "Failed to create store" };
    }
...
export async function getPublicStoresApi(params?: Record<string, any>): Promise<{ success: boolean; data?: any[]; pagination?: any; error?: string }> {
  try {
    ...
    if (!res.ok || !json.success) {
      return { success: false, error: json.error?.message || "Failed to fetch stores" };
    }
...
export async function getStoreByIdApi(id: string): Promise<{ success: boolean; data?: any; error?: string }> {
  try {
    const res = await fetch(`${API_BASE}/${id}`);
    const json = await res.json();
    if (!res.ok || !json.success) {
      return { success: false, error: json.error?.message || "Store not found" };
    }
    return { success: true, data: json.data };
  } catch (error) {
    return { success: false, error: "Network error" };
...
export async function getMyStoresApi(): Promise<{ success: boolean; data?: any[]; error?: string }> {
  try {
    const token = getUserAccessToken();
